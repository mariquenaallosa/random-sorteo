...
/).map((s)=>s.trim()).filter(Boolean);
      ...
      function toast(msg, ms = 1200) {
        ...
        el.textContent = msg;
        Object.assign(el.style, { position:"fixed", bottom:"16px", left:"50%", transform:"translateX(-50%)", padding:"8px 12px", background:"rgba(255,255,255,0.9)", color:"#111", borderRadius:"12px", fontSize:"12px", zIndex:9999 });
        ...
        return (
          <div className="rounded-2xl bg-neutral-800 p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              {items.length > 0 && <div className="text-xs text-neutral-400">Click en 🗑️ para quitar manualmente</div>}
            </div>
            {items.length === 0 ? (
      ...
      function App() {
        ...
        const clearAll = () => {
          if (!confirm("¿Reiniciar todo? Se perderá el historial de sorteos.")) return;
            ...
            <div className="max-w-6xl mx-auto grid gap-6">
              <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button onClick={clearAll} className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-neutral-800 hover:bg-neutral-700">🔄 Reiniciar</button>
                  <button onClick={undo} className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-neutral-800 hover:bg-neutral-700">↩️ Deshacer</button>
                  <button onClick={exportCSV} className="inline-flex items-center gap-2 px-3 py-2 rounded-2xl bg-neutral-800 hover:bg-neutral-700">⬇️ Exportar CSV</button>
              ...
              <section className="grid md:grid-cols-2 gap-6">
                <div className="rounded-2xl bg-neutral-800 p-4">
                  <div className="flex items-center justify-between mb-2">
                    <h2 className="font-semibold">Pegar lista (uno por línea)</h2>
                    <button onClick={addItemsFromText} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white text-neutral-900 font-medium hover:bg-white/90">➕ Cargar</button>
                  </div>
                  <textarea
...
Carla"}
                  ...
                  />
                  <div className="text-xs text-neutral-400 mt-2">Se eliminan duplicados y líneas vacías automáticamente.</div>
                </div>

                <div className="rounded-2xl bg-neutral-800 p-4 grid gap-4">
                  <div>
                    <div className="text-sm text-neutral-300 mb-1">Agregar elemento rápido</div>
                    <div className="flex gap-2">
                      ...
                      />
                      <button onClick={addSingleItem} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white text-neutral-900 font-medium hover:bg-white/90">➕ Agregar</button>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <div className="text-xs text-neutral-400 mt-1">Si defines una semilla y el orden de la lista es el mismo, el resultado será reproducible.</div>
                  </div>
              ...
              <section className="grid md:grid-cols-2 gap-6">
                <ListPanel
                  ...
                  items={state.items}
                  emptyHint="Cargá ítems desde el panel de la izquierda"
                  onRemove={removeItem}
                />
                <ListPanel
                  title={`Sorteados (${state.drawn.length})`}
                  items={state.drawn}
                  emptyHint="Aún no hay sorteados"
                  onRemove={removeDrawn}
                />
              </section>

              <footer className="text-center text-xs text-neutral-500 py-8">
                Hecho con ❤️. Guardado automático en tu navegador.
              </footer>
            </div>
          </div>
        );
